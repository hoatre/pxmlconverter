    ...
    public static void importFile(File file) {
    	try {
			...
			handler.instantiateTables(
					new Table(PREFIX_ACCEL, filename.substring(0, filename.lastIndexOf(DOT)), "",  file.getParent()),
					new Table(PREFIX_CONTENT, filename.substring(0, filename.lastIndexOf(DOT)), "",  file.getParent()),
					new Table(PREFIX_CP, filename.substring(0, filename.lastIndexOf(DOT)),  "", file.getParent())
			);
